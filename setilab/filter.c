...
int convolve_and_compute_power(int length, double input_signal[], 
			       int order, double coeffs[], 
			       int start, int end, double *part_pow_sum)
{
  int i,j;
  double cur_sum1, cur_sum2, cur_sum3, cur_sum4, cur_sum;
  double pow_sum;
  /*int L = 0;
  int n = end - (start + order);
  int m = order + 1;
  m=n+m;
  for (n=1;n<=m;n<<=1) L++;*/
  ...
  for (i = start+order; i<end; i += 4) {
    cur_sum1 = 0;
    cur_sum2 = 0;
    cur_sum3 = 0;
    cur_sum4 = 0;
    for (j=0;j<=order;j++) {
      cur_sum1 += input_signal[i-j] * coeffs[j];
      cur_sum2 += input_signal[i-j+1] * coeffs[j];
      cur_sum3 += input_signal[i-j+2] * coeffs[j];
      cur_sum4 += input_signal[i-j+3] * coeffs[j];
    }
    pow_sum += cur_sum1 * cur_sum1;
    pow_sum += cur_sum2 * cur_sum2;
    pow_sum += cur_sum3 * cur_sum3;
    pow_sum += cur_sum4 * cur_sum4;
  }
  for (i = start; i < start+order; i++) {
    cur_sum = 0;
    for (j=order;j>=0;j--) {        
      if((i-j)>=0){ 
